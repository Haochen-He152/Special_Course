...
XY_RANDOM_OFFSET_RANGES = {
    "fixed": {"x": (0.0, 0.0), "y": (0.0, 0.0)},
    "single": {"x": (-0.08, 0.08), "y": (-0.15, 0.15)},
}
...
@configclass
class SmoothnessPenaltyCurriculumCfg(CurriculumCfg):
...
class LiftEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self._set_yaw_random_size(self.yaw_random_size)
    ...
    def _set_yaw_random_size(self, size: str):
        """Apply a named yaw reset randomization range to the object reset event."""
        if size not in YAW_RANDOM_RANGES:
            valid_sizes = ", ".join(sorted(YAW_RANDOM_RANGES))
            raise ValueError(f"Unknown yaw_random_size '{size}'. Valid sizes: {valid_sizes}.")

        pose_range = self.events.reset_object_position.params["pose_range"]
        pose_range["yaw"] = YAW_RANDOM_RANGES[size]
